...
static lv_style_prop_t button_props[] = { LV_STYLE_OUTLINE_WIDTH,
...
{
    lv_style_transition_dsc_init(&slider_trans_dsc, slider_props,
                                 lv_anim_path_linear, 300, 0, NULL);

    lv_style_init(&slider_style_main);
    lv_style_set_bg_opa(&slider_style_main, LV_OPA_COVER);
    lv_style_set_bg_color(&slider_style_main,
    ...
    lv_style_set_bg_color(&slider_style_indicator,
    ...
    lv_style_set_bg_color(&slider_style_knob,
                          lv_palette_darken(LV_PALETTE_RED, 3));
    lv_style_set_border_color(&slider_style_knob,
                              lv_palette_darken(LV_PALETTE_RED, 5));
    ...
    lv_style_set_bg_color(&slider_style_pressed_color,
...
static void lcd_button_style_init(void)
{
    ...
    lv_style_set_outline_color(&button_style,
                               lv_palette_darken(LV_PALETTE_DEEP_PURPLE,
                                                 3));

    lv_style_set_text_color(&button_style,
    ...
    lv_style_set_bg_color(&button_style_pressed,
                          lv_palette_darken(LV_PALETTE_DEEP_PURPLE,
                                            2));
    ...
    lv_style_transition_dsc_init(&button_trans_dsc, button_props,
                                 lv_anim_path_linear, 100, 0, NULL);
...
static void lcd_voice_icon_style_init(void)
{
    ...
    lv_style_set_text_color(&voice_icon_style,
                            lv_palette_darken(LV_PALETTE_GREEN, 4));
}

static void lcd_mute_icon_style_init(void)
{
    lcd_common_voice_style_init(mute_icon_style);
    lv_style_set_text_color(&mute_icon_style,
                            ...
                            lv_event_cb_t cb)
{
    ...
    lv_obj_add_style(slider, &slider_style_pressed_color,
